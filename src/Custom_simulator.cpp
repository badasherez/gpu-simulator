...
// Group sum function
Gfloat Custom_simulator::group_sum(const std::vector<Gfloat>& array) {
    // Calculate shift value from Custom to FP32 significand width
    constexpr int custom_to_fp32_shift = CUSTOM_SIGNIFICAND_WIDTH - FP32_SIGNIFICAND_WIDTH;
    
    // Step 1: Find the maximal exponent of the Gfloats
    int16_t max_exponent = CUSTOM_ZERO_EXPONENT;
    ...
    for (const auto& gfloat : array) {
        ...
        if (gfloat.get_sign()) {
            significand -= (gfloat.get_significand() << custom_to_fp32_shift) >> (max_exponent - gfloat.get_exponent());
        } else {
            significand += (gfloat.get_significand() << custom_to_fp32_shift) >> (max_exponent - gfloat.get_exponent());
        }
    }
    ...
    if (width == 0) {
        return Gfloat(sign, CUSTOM_ZERO_EXPONENT, 0);
    }
    int16_t exp = max_exponent + width - CUSTOM_SIGNIFICAND_WIDTH;
    if (width > CUSTOM_SIGNIFICAND_WIDTH) {
        significand = significand >> (width - CUSTOM_SIGNIFICAND_WIDTH);
    }
    else {
        significand = significand << (CUSTOM_SIGNIFICAND_WIDTH - width);
    }
    if (exp < FP32_MIN_NONZERO_EXPONENT) {
        significand = significand >> (FP32_MIN_NONZERO_EXPONENT - exp);
        exp = FP32_MIN_NONZERO_EXPONENT;
    }
    significand = significand >> custom_to_fp32_shift;
    if (significand == 0) {
        return Gfloat(sign, CUSTOM_ZERO_EXPONENT, 0);
    }
...
template<typename T>
torch::Tensor Custom_simulator::matrix_multiply_template(const torch::Tensor& A, const torch::Tensor& B, 
    ...
    auto compute_range = [&](int start_element, int end_element) {
        for (int element_idx = start_element; element_idx < end_element; ++element_idx) {
            ...
            accumulator.reserve(CUSTOM_ACCUMULATOR_GROUP_SIZE);
            accumulator.push_back(Gfloat(false, CUSTOM_ZERO_EXPONENT, 0));
            ...
            for (int l = 0; l < k; ++l) {
                ...
                if (accumulator.size() == CUSTOM_ACCUMULATOR_GROUP_SIZE) {
...
torch::Tensor Custom_simulator::matmul_bfloat16(const torch::Tensor& A, const torch::Tensor& B) {
    return matrix_multiply_template<c10::BFloat16>(A, B, torch::kBFloat16, "bfloat16", multiply_bfloat16_to_gfloat, CUSTOM_ZERO_EXPONENT);
}

// Matrix multiplication using float16
torch::Tensor Custom_simulator::matmul_float16(const torch::Tensor& A, const torch::Tensor& B) {
    return matrix_multiply_template<c10::Half>(A, B, torch::kFloat16, "float16", multiply_float16_to_gfloat, CUSTOM_ZERO_EXPONENT);
}

// Unified matrix multiplication
torch::Tensor Custom_simulator::matmul(const torch::Tensor& A, const torch::Tensor& B) {
    if (A.dtype() != B.dtype()) {
